...
void VoxInstance::generateMesh2(uint32& vertexCount, uint32 meshingSSBO, vec4 offset, ivec3 modelSize, ComputeShader& compute, float64& dispatchDuration)
{
    DrawArraysIndirectCommand indirectData{};
    indirectData.count = 0;
    indirectData.instanceCount = 1;
    ...
    InstanceData instanceData{};
    instanceData.bufferSize = 0; // Calculated in meshing step
    instanceData.instanceSize = vec4(modelSize, 0);
    instanceData.offset = offset;
    instanceData.voxelCount = 0;
    ...
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 1, indirectCommand);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 2, instanceDataBuffer);
